export function getStatusHtml(): string {
  return [
    ...
    '        <article class="panel status"><p class="label">Registered Users</p><p class="value warn" id="users-value">Checking...</p></article>',
    '        <article class="panel status"><p class="label">Demo Posts</p><p class="value warn" id="posts-value">Checking...</p></article>',
    '      </section>',
    ...
    '        try {',
    '          const results = await Promise.all([fetchJson("/api/health"), fetchJson("/api/mock-data")]);',
    ...
    '          setStatus(els.usersValue, userCount + " users", "good");',
    '          setStatus(els.postsValue, postCount + " posts", "good");',
